...
def generate_rationale(args):
    data_path = f"/hdd/hdd1/guest-pjy/InstrctRAG_dataset/{args.dataset_name}/train.json"
...
def eval_model(args):
    data_path = f"/hdd/hdd1/guest-pjy/InstructRAG_dataset/{args.dataset_name}/test.json"
    ...
    if args.rag_model == "InstructRAG-FT":
        ...
        demos = common_utils.jload(
            f"/hdd/hdd1/guest-pjy/InstructRAG_dataset/{args.dataset_name}/demos.json"
        )
        llm = LLM(
    ...
    get_metrics(
        eval_results, args.output_dir, is_asqa=args.dataset_name == "ASQA"
    )
        ...
        output_data.append(
            {
                ...
                "qa_pairs": (
                    sample["qa_pairs"] if "qa_pairs" in sample else None
                ),
                ...
                "ctxs": (
                    ...
                    if (
                        sample["ctxs"][0]["score"] > sample["ctxs"][1]["score"]
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", type=str, help="Path to the output file"
    )
    parser.add_argument(
        "--cache_dir",
        type=str,
