...
def build_contexts(example, n_docs):
    ...
    if (
        len(example["ctxs"]) > 0
        and example["ctxs"][0]["score"] > example["ctxs"][1]["score"]
    ):
...
def preprocess_for_rag(
    ...
    assist_prefix_len = len(
        tokenizer.encode(
            assistant_prefix, add_special_tokens=False, return_tensors="pt"
        )[0]
    )

    user_prefix = prompt_dict["user_prefix"]
    user_prefix_id = tokenizer.encode(
        user_prefix, add_special_tokens=True, return_tensors="pt"
    )[0]
    user_prefix_len = len(user_prefix_id)

    for sample in data_list:
        query_prompt = prompt_dict["query_prompt"] + normalize_question(
            sample["question"]
        )
        ...
        target_prompt = (
            assistant_prefix + sample["rationale"] + tokenizer.eos_token
        )
    ...
    examples_tokenized = _tokenize_fn(
        examples,
        tokenizer,
        max_len_offset=[user_prefix_len] * len(examples),
        add_special_tokens=False,
    )

    input_ids = [
        torch.cat([user_prefix_id, ctx])
        for ctx in examples_tokenized["input_ids"]
    ]
    targets_tokenized = _tokenize_fn(
        targets, tokenizer, add_special_tokens=False
    )
    ...
    for idx, label in enumerate(labels):
        ...
        assert torch.all(
            labels[idx][-(target_len - assist_prefix_len) :].eq(
                targets_tokenized["input_ids"][idx][assist_prefix_len:]
            )
        )
...
def format_prompt(
    ...
    if do_rationale_generation:
        ...
        target_prefix += (
            ...
            + prompt_dict["rationale_generation_postfix_" + dataset_name]
        )
